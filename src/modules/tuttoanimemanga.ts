...
class TuttoAnimeManga extends Module {
  ...
  search(query: string): Promise<SearchResult[]> {
    return new Promise(async (resolve: (value: SearchResult[]) => void) => {
      ...
      const page = await axios.get(query === '' ? 'https://tuttoanimemanga.net/api/comics' : 'https://tuttoanimemanga.net/api/search/'+encodeURIComponent(query))

      resolve(page.data.comics.map((v: { url: string; title: string }) => ({
        id: v.url,
        title: v.title
      })))
  ...
  manga(id: string): Promise<Manga> {
    return new Promise(async (resolve: (value: Manga) => void) => {
      ...
      resolve({
        ...
        artist: comic.artist,
        img: comic.thumbnail,
        chapters: comic.chapters.map((v: { full_title: string; url: string; published_on: string }) => ({
          title: v.full_title,
          id: v.url,
