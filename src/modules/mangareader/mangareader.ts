...
class MangaReader extends Module {
  ...
  search(query: string) {
    return new Promise(async (resolve: (value: SearchResult[]) => void) => {
      ...
      const $ = cheerioload((await axios.get(`https://mangareader.to/search?keyword=${query}`)).data)

      let results: SearchResult[] = []
      $('#main-content > section > div.manga_list-sbs > div.mls-wrap').find('div.item.item-spc').each((_, el) => {
        const ele = $(el)
  ...
  manga(mangaid: string) {
    return new Promise(async (resolve: (value: Manga) => void) => {
      ...
      $('#'+lang+'-chapters').find('a').each((_, ch) => {
        const chap = $(ch)
        chapters.push({
      ...
      resolve({
        ...
        author: $('#ani_detail > div > div > div.anis-content > div.anisc-detail > div.sort-desc > div.anisc-info-wrap > div.anisc-info > div:nth-child(3)').text().split('Authors:')[1] || 'unknown',
        artist: '',
        img: $('#ani_detail > div > div > div.anis-content > div.anisc-poster > div > img').attr('src')+'',
        chapters,
  ...
  chapter(manga: string, id: string) {
    return new Promise(async (resolve: (value: string[]) => void) => {
      ...
      $ = cheerioload((await axios.get(`https://mangareader.to/ajax/image/list/chap/${chapid}?mode=horizontal&quality=high&hozPageSize=1`)).data.html+'')
