...
class Onepiecepower extends Module {
	...
	manga(mangaid: string) {
		return new Promise(async (resolve: (value: Manga) => void) => {
			...
			await Promise.all(
				parsed('body > table > tbody > tr:nth-child(5) > td')
					.find('a')
					.each((i, e) => {
						const el = parsed(e);
			...
			let aut = '',
				syn = '';
			parsed('body > table > tbody > tr:nth-child(3) > td')
				.find('span')
				.each((i, e) => {
					const el = parsed(e);
					if (el.text() === 'Autore:') {
	...
	private async pageExist(baseurl: string, page: number) {
		return (
			(await axios.get(this.pageUrl(baseurl, page), { validateStatus: () => true })).status === 200
		);
	}
}
